...
export const getLocalizedImagePath = (
  baseImage: string | { src: string },
  locale: string,
): string => {
  // Если это импортированное изображение Next.js, получаем его src
  const imageSrc =
    typeof baseImage === "string" ? baseImage : baseImage.src || baseImage;

  if (typeof imageSrc !== "string") {
    console.warn(
      "getLocalizedImagePath: baseImage is not a string or valid image object",
    );
